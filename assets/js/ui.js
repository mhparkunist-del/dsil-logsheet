/* =====================================================================
   DSIL Log Sheet – shared UI helpers (Tabler markup)
   Used by portal.js, forms.js and sheet.js. Exposes window.DSILUI.
   ===================================================================== */
(function () {
  'use strict';
  ...
  /* data-show-if="name=value" : 같은 폼 안의 컨트롤 값이 맞을 때만 보이고, 숨겨진 블록의 입력은 disabled 로 검증·수집에서 제외 */
  function applyShowIf(root) {
    $all('[data-show-if]', root || document).forEach(function (el) {
      var spec = el.getAttribute('data-show-if').split('=');
      var name = spec[0], want = spec.slice(1).join('=');
  ...
  document.addEventListener('change', function (e) { var f = e.target.closest && e.target.closest('form'); if (f && f.querySelector('[data-show-if]')) applyShowIf(f); });
  document.addEventListener('input', function (e) { if (e.target.type === 'radio') { var f = e.target.closest('form'); if (f && f.querySelector('[data-show-if]')) applyShowIf(f); } });
  ...
  function dialog(opts) {
    return new Promise(function (resolve) {
      ...
      var inner = '';
      if (opts.bodyHtml) inner = '<form id="modal-form" novalidate="false">' + opts.bodyHtml + '</form>';
      ...
      wrap.innerHTML = '<div class="modal modal-blur fade show is-open" tabindex="-1" role="dialog" aria-modal="true" aria-labelledby="modal-title">'
        + '<div class="modal-dialog ' + (opts.size === 'lg' ? 'modal-lg' : (opts.size === 'md' ? '' : 'modal-sm')) + ' modal-dialog-centered modal-dialog-scrollable" role="document"><div class="modal-content">'
      ...
      var first = input || (form && form.querySelector('input:not([disabled]):not([type="hidden"]),select:not([disabled]),textarea:not([disabled])')) || wrap.querySelector('[data-modal="ok"]');
  ...
  window.DSILUI = {
    nf: nf, esc: esc, pad2: pad2, localDate: localDate, fmtDate: fmtDate, fmtTime: fmtTime, fmtDateTime: fmtDateTime,
    toLocalInput: toLocalInput, fromLocalInput: fromLocalInput, fmtDuration: fmtDuration,
    $: $, $all: $all, toast: toast, readForm: readForm, applyShowIf: applyShowIf, dialog: dialog, confirmDlg: confirmDlg, promptDlg: promptDlg, closeAll: closeAll,
