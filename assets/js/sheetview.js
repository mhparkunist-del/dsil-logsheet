...
(function () {
  ...
  var SH = window.DSILSheet, U = window.DSILUI, esc = U.esc;
  ...
  /* 분기점 추가 폼: 시작 행 · 끝 행 · 나눌 구간(시작 행의 잎) · 분기 목록 */
  function splitAddBody(sheet, unitLabel, preFromRowId) {
    ...
    var from = preFromRowId && SH.rowById(sheet, preFromRowId) ? preFromRowId : sheet.rows[0].id;
    var html = '<div class="text-secondary small mb-2">분기점은 <b>행 범위</b>에 걸칩니다. 범위 안에서는 분기마다 셀(스텝)을 따로 두거나 비워서 건너뛰고, 끝 행 다음부터 다시 합쳐집니다. 분기 수량의 합은 나누는 구간의 수량과 같아야 합니다.</div>'
      ...
      + '<div class="col-6"><label class="form-label">끝 행</label><select class="form-select" name="toRowId">' + rowOptions(sheet, '', { none: '끝까지 (합치지 않음)' }) + '</select></div></div>';
    ...
    var n = sheet.unitCount;
    html += '<div class="d-flex justify-content-between align-items-center mt-3 mb-1"><div class="subheader mb-0">분기</div><button type="button" class="btn btn-sm" data-action="brow-add"><i class="ti ti-plus me-1"></i>분기</button></div><div id="brow-box" data-unit="' + esc(unitLabel) + '">' + branchRows([{ name: 'A', count: Math.ceil(n / 2) }, { name: 'B', count: Math.floor(n / 2) || 1 }], unitLabel) + '</div>';
    return html;
  }
  ...
  function splitEditBody(sheet, split, unitLabel) {
    var r = SH.range(sheet, split);
    return '<div class="text-secondary small mb-2">' + (split.parentId === 'all' ? '공통 구간' : '상위 분기 안') + '을 나누는 분기점입니다. 범위를 줄이면 빠진 행의 분기 셀은 하나로 합쳐지고, 늘리면 상위 셀이 분기마다 복제됩니다. 스텝이 진행된 분기는 뺄 수 없습니다.</div>'
