/* =====================================================================
   DSIL Run Sheet – 이력: 내 런 / 아카이브(공개) / 전체(관리자)의 기록·시트 수정을 팀 / 이름 / 공정 일자 / 런 / 동작별로 묶어 열람, CSV
   ===================================================================== */
(function () {
  'use strict';
  ...
  var state = { ready: false, error: null, runs: [], logs: [], scope: 'mine', filter: { domain: '', team: '', who: '', runId: '', kind: 'all', preset: '30', from: '', to: '', q: '' }, group: 'date' };
  ...
  function teams() { var set = {}; state.runs.forEach(function (r) { if (r.team) set[r.team] = 1; }); state.logs.forEach(function (l) { if (l.team) set[l.team] = 1; }); return Object.keys(set).sort(); }
  ...
  function reload() { return Promise.all([store.listRuns({ scope: state.scope }), store.listLogs(null, 8000, state.scope)]).then(function (r) { state.runs = r[0]; state.logs = r[1]; }); }
  ...
  function logRow(l) {
    ...
    return '<tr><td class="text-nowrap text-secondary small">' + fmtDateTime(l.at) + '</td><td class="text-nowrap small">' + esc(l.date || localDate(l.at)) + '</td>'
      + '<td class="small">' + (r ? '<a href="../run/index.html#id=' + esc(r.id) + '">' + esc(r.code) + '</a><br><span class="text-secondary">' + esc(r.title) + '</span>' : '<span class="text-secondary">-</span>') + '</td>'
  ...
  function renderResults() {
    ...
    if (!logs.length) return html + '<div class="card">' + empty('history-off', '조건에 맞는 기록이 없습니다', state.scope === 'mine' ? '내가 만든 런의 기록만 보입니다. 다른 팀의 공개 런은 "아카이브"에서 보세요.' : '') + '</div>';
    if (state.group === 'none') return html + '<div class="card"><div class="table-responsive"><table class="table table-sm table-vcenter card-table">' + head + '<tbody>' + logs.slice(0, 1000).map(logRow).join('') + '</tbody></table></div></div>';
    var groups = {}, order = [];
    ...
    return html + order.map(function (k) {
      ...
      return '<div class="card mb-3"><div class="card-header"><h3 class="card-title"><i class="ti ti-' + icon + ' me-1 text-primary"></i>' + esc(k) + '</h3><div class="card-actions text-secondary small">' + sub + '</div></div><div class="table-responsive"><table class="table table-sm table-vcenter card-table">' + head + '<tbody>' + list.slice(0, 500).map(logRow).join('') + '</tbody></table></div></div>';
    }).join('');
  }
  function render() {
    ...
    var f = state.filter, m = me();
    var scopes = [['mine', '내 런'], ['archive', '아카이브 (공개)']].concat(m.isAdmin ? [['all', '전체 (관리자)']] : []);
    var html = '<div class="card mb-3"><div class="card-body"><form id="hist-filter" class="row g-2 align-items-end">'
      + '<div class="col-6 col-md-2"><label class="form-label">범위</label><select class="form-select" name="scope">' + scopes.map(function (s) { return '<option value="' + s[0] + '"' + (state.scope === s[0] ? ' selected' : '') + '>' + s[1] + '</option>'; }).join('') + '</select></div>'
      ...
      + '<div class="col-6 col-md-2"><label class="form-label">런</label><select class="form-select" name="runId"><option value="">전체</option>' + state.runs.map(function (r) { return '<option value="' + esc(r.id) + '"' + (f.runId === r.id ? ' selected' : '') + '>' + esc(r.code + ' ' + r.title) + '</option>'; }).join('') + '</select></div>'
      + '<div class="col-6 col-md-2"><label class="form-label">종류</label><select class="form-select" name="kind">' + KINDS.map(function (k) { return '<option value="' + k[0] + '"' + (f.kind === k[0] ? ' selected' : '') + '>' + k[1] + '</option>'; }).join('') + '</select></div>'
  ...
  document.addEventListener('change', function (e) {
    ...
    var v = readForm(form); var scopeChanged = v.scope !== state.scope; state.scope = v.scope || 'mine';
    state.filter = { domain: v.domain || '', team: v.team || '', who: v.who || '', runId: v.runId || '', kind: v.kind || 'all', preset: v.preset || '30', from: v.from || '', to: v.to || '', q: v.q || '' }; state.group = v.group || 'date';
    if (scopeChanged) { state.filter.runId = ''; refresh(); } else render();
  });
  ...
  (function () { try { var q = new URLSearchParams(window.location.search); ['team', 'who', 'runId', 'domain'].forEach(function (k) { if (q.get(k)) state.filter[k] = q.get(k); }); if (q.get('group')) state.group = q.get('group'); if (q.get('scope')) state.scope = q.get('scope'); } catch (e) { /* ignore */ } })();
  store.init().then(function () {
    if (!store.getSession()) { window.location.replace('../index.html?next=history'); return; }
    state.ready = true; store.onChange(function () { reload().then(render).catch(handleError); }); return refresh();
