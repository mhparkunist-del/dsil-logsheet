/* =====================================================================
   DSIL Run Sheet – 공정 런: 내 런 / 아카이브(모두 공개, 읽기 전용) / 전체(관리자) 목록 + 런시트(스플릿 표)
   런시트 편집 모드에서 라이브러리 모듈·흐름을 바로 불러와 처음부터 구성. 완료 시 아카이브 올리기(동의한 것만).
   해시: #id=<runId>[&step=<stepId>][&edit=1]  |  #scope=mine|archive|all
   ===================================================================== */
(function () {
  'use strict';
  ...
  function stepCell(s, cell, pg) {
    var r = state.run, active = r.status === 'active' && !ro(), isCur = !!(pg.current && pg.current.id === s.id);
    var hasActual = Object.keys(s.actual || {}).length > 0;
    var html = '<div class="step-cell">'
      + '<div class="d-flex justify-content-between align-items-start gap-2"><div><span class="step-num is-' + s.status + '">' + s.seq + '</span> <b class="step-name">' + esc(s.name) + '</b>' + (s.addedInRun && r.flowId ? ' <span class="badge bg-purple-lt">런 중 추가</span>' : '')
  ...
  function cellHtml(cell, pg) {
    var r = state.run, span = cell.count > 1 ? ' colspan="' + cell.count + '"' : '', edit = state.editSheet && !ro();
  ...
  function renderPalette() {
    var r = state.run, dom = r.domain, cats = CFG.categories || [];
    ...
    var flowOpts = fo.map(function (f) { return '<option value="' + esc(f.id) + '">' + (f.domain !== dom ? '[' + esc(domainInfo(f.domain).short) + '] ' : '') + esc(f.name) + (S.hasSplit(f.items) ? ' [분기 · ' + esc(f.unitLabel) + ' ' + f.unitCount + ']' : ' (' + f.items.length + '항목)') + (f.seed ? ' · 기본 제공' : f.ownerName ? ' · ' + esc(f.ownerName) : '') + '</option>'; }).join('');
    return '<div class="card mb-3 border-purple"><div class="card-body"><div class="d-flex align-items-center flex-wrap gap-2 mb-2"><i class="ti ti-books text-purple"></i><b>라이브러리에서 추가</b><span class="text-secondary small">모듈은 계획 조건을 기본값으로 넣고 연필 버튼으로 고칩니다. 흐름 복사는 분기점까지 그대로 가져오며, 빈 런이면 수량도 흐름에 맞춥니다. 특정 위치에 넣으려면 스텝의 <i class="ti ti-row-insert-top"></i> 버튼이나 분기의 "+ 스텝"을 쓰세요.</span></div>'
      + '<div class="row g-2"><div class="col-lg-6"><label class="form-label">모듈 <span class="form-label-description">끝에 추가</span></label><div class="input-group"><select class="form-select" id="pal-module">' + modOpts + '</select><button type="button" class="btn btn-primary" data-action="pal-module"><i class="ti ti-plus me-1"></i>추가</button><button type="button" class="btn" data-action="pal-module-detail" title="계획 조건·이름을 정하고 추가"><i class="ti ti-adjustments"></i></button></div></div>'
  ...
  function renderSheet() {
    var r = state.run, pg = S.progress(r), edit = state.editSheet && !ro();
    var html = edit ? renderPalette() : '';
    if (!r.steps.length && !S.hasSplit(r.tree)) return html + '<div class="card mb-3">' + empty('list', '아직 스텝이 없습니다', edit ? '위의 라이브러리에서 모듈을 추가하거나 흐름을 통째로 복사하세요.' : '"시트 편집"을 눌러 라이브러리에서 모듈·흐름을 넣으세요.') + (!edit && !ro() ? '<div class="text-center pb-4"><button type="button" class="btn btn-primary" data-action="toggle-edit"><i class="ti ti-list-details me-1"></i>시트 편집</button></div>' : '') + '</div>';
    var rows = L.rows(L.resolve(r.tree, stepsById()), r.unitCount, { foot: edit });
    html += '<div class="card mb-3"><div class="table-responsive"><table class="table card-table split-table"><thead><tr>' + r.units.map(function (u) { return '<th class="unit-col">' + esc(u) + '</th>'; }).join('') + '</tr></thead><tbody>';
    rows.forEach(function (row) { html += '<tr class="row-' + row.type + '">' + row.cells.map(function (c) { return cellHtml(c, pg); }).join('') + '</tr>'; });
    html += '</tbody></table></div>';
    if (edit) html += '<div class="card-footer d-flex flex-wrap gap-2"><button type="button" class="btn btn-sm" data-action="sheet-insert" data-container="root" data-index="' + r.tree.length + '"><i class="ti ti-plus me-1"></i>끝에 스텝 추가 (조건 지정)</button><button type="button" class="btn btn-sm" data-action="split-add" data-container="root" data-index="' + r.tree.length + '"' + (r.unitCount < 2 ? ' disabled title="수량이 1이라 나눌 수 없습니다"' : '') + '><i class="ti ti-git-branch me-1"></i>끝에 분기점 추가</button><span class="text-secondary small ms-auto">분기점 뒤의 스텝은 합쳐진(merge) 공정입니다.</span></div>';
    return html + '</div>';
  }
  function renderDetail() {
    var r = state.run, pg = S.progress(r), cur = pg.current, active = r.status === 'active' && !ro(), m = me();
    var html = '';
    ...
    html += '<div class="card mb-3"><div class="card-body"><div class="row g-3 align-items-center">'
      ...
      + '</div></div></div></div></div>';
    if (state.editSheet && !ro()) html += '<div class="alert alert-info py-2 px-3"><i class="ti ti-info-circle me-1"></i><b>시트 편집 모드</b> — 라이브러리에서 모듈·흐름을 불러오고, 스텝 순서·계획 조건·분기점을 바꿉니다. 모든 변경은 변경 이력에 남습니다.</div>';
    html += renderSheet();
  ...
  function exportCSV() {
    var r = state.run;
    var head = ['순번', '분기', '그룹', '공정', '장비', '상태', '작업자', '팀', '공정 일자', '계획 조건', '실제 조건', '결과', '특이사항', '비고'];
    var lines = [head.map(csvCell).join(',')].concat(r.steps.map(function (s) { return [s.seq, (s.branchPath || []).join(' › '), s.group, s.name, s.equipment, S.STEP_STATUS[s.status], s.operator, s.team, s.date || '', P.text(s.fields, s.planned), P.text(s.fields, s.actual), s.result, s.issues, s.note].map(csvCell).join(','); }));
  ...
  function stepDialog(s) {
    var m = me(), readOnly = ro(), actual = Object.keys(s.actual || {}).length ? s.actual : s.planned;
    ...
    var body = '<div class="d-flex flex-wrap justify-content-between gap-2 mb-2"><div>' + (s.branchPath && s.branchPath.length ? '<span class="step-group">' + esc(s.branchPath.join(' › ')) + '</span> · ' : '') + (s.group ? '<span class="text-secondary small">' + esc(s.group) + ' › </span>' : '') + '<b>' + esc(s.name) + '</b></div><div class="small text-secondary">' + P.catBadge(s.category) + (s.equipment ? ' ' + esc(s.equipment) : '') + '</div></div>'
      ...
      + '<div class="form-hint" data-show-if="status=skipped">건너뛴 이유를 특이사항에 적어 주세요.</div><div class="form-hint" data-show-if="status=failed">실패 원인·조치를 특이사항에 적어 주세요. 재작업은 시트 편집에서 스텝을 추가합니다.</div></fieldset>';
    var p = dialog({ title: '스텝 기록 · ' + s.seq + '. ' + s.name, bodyHtml: body, okLabel: readOnly ? '닫기' : '저장', size: 'lg', hideCancel: readOnly });
    loadThumbs();
  ...
  function insertDialog(container, index, preModule) {
    ...
    return dialog({ title: '런시트에 스텝 추가', bodyHtml: body, okLabel: '추가', size: 'lg' }).then(function (v) {
      if (!v) return null;
      if (v.kind === 'flow') return store.sheetInsert(r.id, { container: container, index: index, flowId: v.flowId, note: v.note });
      var modId = v.showAll && v.moduleIdAll ? v.moduleIdAll : v.moduleId;
      var mod = modById(modId);
      return store.sheetInsert(r.id, { container: container, index: index, moduleId: modId, name: v.name, params: P.read(mod ? mod.fields : [], v), note: v.note });
    });
  }
  ...
  function splitAddDialog(container, index) {
    ...
    return dialog({ title: '분기점 추가', bodyHtml: body, okLabel: '분기점 만들기', size: 'md' }).then(function (v) { if (!v) return null; return store.splitAdd(r.id, { container: container, index: index, name: v.name, branches: readBranches(v) }); });
  }
  function splitEditDialog(split) {
    ...
    return dialog({ title: '분기 수정', bodyHtml: body, okLabel: '저장', size: 'md' }).then(function (v) { if (!v) return null; return store.splitEdit(r.id, split.id, { name: v.name, branches: readBranches(v) }); });
  }
  function planDialog(s) {
    ...
    return dialog({ title: '계획 수정 · ' + s.seq + '. ' + s.name, bodyHtml: body, okLabel: '저장', size: 'lg' }).then(function (v) { if (!v) return null; return store.sheetEdit(state.run.id, s.id, { name: v.name, equipment: v.equipment, itemNote: v.itemNote, planned: P.read(s.fields, v) }); });
  }
  function runEditDialog(r) {
    ...
    var body = '<div class="row g-2"><div class="col-md-8"><label class="form-label required">제목</label><input type="text" class="form-control" name="title" required value="' + esc(r.title) + '" autocomplete="off"></div><div class="col-md-4"><label class="form-label">시작</label><input type="datetime-local" class="form-control" name="startedAt" value="' + toLocalInput(r.startedAt) + '"></div></div>'
      ...
      + '<div class="col-md-2"><label class="form-label">수량</label><input type="number" class="form-control" name="unitCount" min="1" value="' + r.unitCount + '"' + (S.hasSplit(r.tree) ? ' disabled title="분기점이 있어 바꿀 수 없습니다"' : '') + '></div>'
  ...
  function moreDialog(r) {
    ...
    items.push(['run-to-flow', 'git-branch', '내 라이브러리 흐름으로 저장', '이 런의 스텝·분기·조건을 새 공정 흐름(템플릿)으로']);
    if (owner) {
  ...
  function toFlowDialog(r) {
    var body = '<div class="mb-2"><label class="form-label required">흐름 이름</label><input type="text" class="form-control" name="name" required value="' + esc(r.title) + '" autocomplete="off"></div>'
      ...
      + '<div class="form-hint mt-2">분기점·수량(' + esc(r.unitLabel) + ' ' + r.unitCount + ')도 함께 저장됩니다. 라이브러리에서 내 흐름으로 보이고 나만 고칠 수 있습니다.</div>';
  ...
  function openStep(id) {
    ...
    if (!ro() && state.run.status !== 'active' && !S.TERMINAL[s.status]) { toast('런이 ' + S.RUN_STATUS[state.run.status] + ' 상태라 기록할 수 없습니다. 더 보기에서 재개하세요.', true); return Promise.resolve(); }
    return (ro() ? Promise.resolve(me()) : ensureTeam()).then(function (m) { if (!m) return null; return stepDialog(s); }).then(function (res) { if (res) { toast(res.status === 'done' ? '완료를 기록했습니다.' : S.STEP_STATUS[res.status] + ' 으로 저장했습니다.'); return refresh(); } });
  }
  ...
  function palAdd(kind) {
    ...
    var o = { container: 'root', index: r.tree.length }; if (kind === 'flow') { o.flowId = sel.value; o.whole = true; } else o.moduleId = sel.value;
    ensureTeam().then(function (m) { if (!m) return; return store.sheetInsert(r.id, o).then(function (run) { toast(kind === 'flow' ? '흐름을 복사했습니다.' + (run.unitCount !== r.unitCount ? ' 수량을 ' + run.unitCount + '로 맞췄습니다.' : '') : '스텝을 추가했습니다.'); return refresh(); }); }).catch(handleError);
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    switch (action) {
      ...
      case 'step-quick':
        ensureTeam().then(function (m) { if (!m) return; return store.stepQuick(r.id, id).then(function (s) { toast(s.seq + '. ' + s.name + ' 을 계획대로 완료로 기록했습니다.'); return refresh(); }); }).catch(function (err) { if (/필수/.test(err.message)) { toast(err.message, true); openStep(id).catch(handleError); } else handleError(err); }); break;
      ...
      case 'pal-module-detail': { var pm = $('#pal-module'); ensureTeam().then(function (m) { if (!m) return; return insertDialog('root', r.tree.length, pm ? pm.value : '').then(function (res) { if (res) { toast('스텝을 추가했습니다.'); return refresh(); } }); }).catch(handleError); break; }
      case 'sheet-insert': ensureTeam().then(function (m) { if (!m) return; return insertDialog(el.getAttribute('data-container') || 'root', Number(el.getAttribute('data-index')) || 0, '').then(function (res) { if (res) { toast('스텝을 추가했습니다.'); return refresh(); } }); }).catch(handleError); break;
      case 'sheet-remove': { var sr = stepById(id); if (!sr) break; confirmDlg({ title: '스텝 삭제', message: sr.seq + '. ' + sr.name + ' 스텝을 런시트에서 뺄까요? 변경 이력에 남습니다.', okLabel: '삭제', danger: true }).then(function (ok) { if (!ok) return; return store.sheetRemove(r.id, id).then(function () { toast('스텝을 뺐습니다.'); return refresh(); }); }).catch(handleError); break; }
      case 'sheet-move': store.sheetMove(r.id, id, el.getAttribute('data-dir')).then(function () { return refresh(); }).catch(handleError); break;
      ...
      case 'split-add': ensureTeam().then(function (m) { if (!m) return; return splitAddDialog(el.getAttribute('data-container') || 'root', Number(el.getAttribute('data-index')) || 0).then(function (res) { if (res) { toast('분기점을 추가했습니다. 분기마다 스텝을 넣으세요.'); return refresh(); } }); }).catch(handleError); break;
      ...
      case 'split-remove': confirmDlg({ title: '분기점 삭제', message: '이 분기점을 지울까요? 분기 안에 스텝이 없어야 합니다.', okLabel: '삭제', danger: true }).then(function (ok) { if (!ok) return; return store.splitRemove(r.id, id).then(function () { toast('분기점을 지웠습니다.'); return refresh(); }); }).catch(handleError); break;
      case 'brow-add': { var box = $('#brow-box'); if (box) { var n = box.querySelectorAll('[data-brow]').length; box.insertAdjacentHTML('beforeend', branchRows([{ id: '', name: String.fromCharCode(65 + n), count: 1 }], r ? r.unitLabel : '')); var rows = box.querySelectorAll('[data-brow]'); var last = rows[rows.length - 1]; last.querySelectorAll('input').forEach(function (i) { i.name = i.name.replace(/_(n\d*|)$/, '_n' + n); }); } break; }
