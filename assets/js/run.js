...
(function () {
  ...
  function rowHeadHtml(row, i, leaves) {
    ...
    if (edit) {
      html += '<div class="d-flex flex-wrap gap-1 mt-2">'
        ...
        + (leaves.length > 1 ? '<button type="button" class="btn btn-sm btn-ghost-primary btn-icon" data-action="row-cond" data-id="' + esc(row.id) + '" title="분기별 조건 한 번에 (분기 ' + leaves.length + '개)"><i class="ti ti-adjustments-horizontal"></i></button>' : '')
  ...
  function renderPalette() {
    ...
    return '<div class="card mb-3 border-purple"><div class="card-body"><div class="d-flex align-items-center flex-wrap gap-2 mb-2"><i class="ti ti-books text-purple"></i><b>라이브러리에서 추가</b><span class="text-secondary small">모듈은 <b>공통 행(큰 스텝)</b>으로 끝에 붙습니다. 끝까지 열린 분기점이 있으면 그 앞 행에서 합쳐집니다(분기마다 따로 넣으려면 <i class="ti ti-adjustments"></i> 상세 추가에서 "모든 분기"). 흐름 복사는 행·분기점을 그대로 가져오며 빈 런이면 수량도 흐름에 맞춥니다. 분기점은 행의 <i class="ti ti-git-branch"></i>, 합침은 행의 <i class="ti ti-arrows-join"></i>, <b>분기별 조건 한 번에</b>는 행의 <i class="ti ti-adjustments-horizontal"></i> 버튼.</span></div>'
  ...
  function rowCondDialog(row) {
    var r = state.run, i = SH.idx(r, row.id), leaves = SH.leavesAt(r, i);
    var cols = leaves.map(function (l) { var c = row.cells[l.id], mod = c ? modById(c.moduleId) : null; return { id: l.id, label: (l.path.length ? l.path.join(' › ') : '공통') + ' (' + l.count + ' ' + r.unitLabel + ')', present: !!c, locked: !!c && c.status !== 'pending', lockedText: c ? S.STEP_STATUS[c.status] : '', name: c ? c.name : '', fields: c ? c.fields : [], values: c ? c.planned : {}, modName: c ? (mod ? mod.name : c.name) : '', moduleId: c ? c.moduleId : null }; });
    ...
    var o = sharedMod ? { sharedFields: present[0].fields, sharedModuleId: sharedMod.id, sharedModName: sharedMod.name, sharedDefaults: S.defaults(sharedMod.fields) } : {};
    o.intro = '<b>' + (i + 1) + '행 "' + esc(row.name) + '"</b> 의 분기 ' + leaves.length + '개 조건을 한 번에 정합니다. ' + (sharedMod ? '열 = 분기, 행 = 조건 항목. 비어 있는 분기는 "채우기"로 같은 모듈을 넣고, "이 행 건너뜀"으로 뺍니다. 진행된 스텝은 바꿀 수 없습니다.' : '분기마다 모듈이 달라 분기별로 나눠 보여 줍니다. (다른 모듈로 바꾸려면 셀을 비우고 "+ 모듈")');
    return dialog({ title: '분기별 조건 · ' + (i + 1) + '. ' + row.name, bodyHtml: SV.rowCondBody(cols, o), okLabel: '저장', size: 'lg', submit: function (v) { return store.rowCells(r.id, row.id, SV.readRowCond(v, cols, o)); } });
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    switch (action) {
      ...
      case 'row-cond': { var rcd = SH.rowById(r, id); if (!rcd) break; rowCondDialog(rcd).then(done('분기별 조건을 저장했습니다.')).catch(handleError); break; }
