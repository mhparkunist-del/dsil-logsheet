/* =====================================================================
   ...
   흐름 items: {kind:'module'|'flow', refId, label, note, params} 또는
              {kind:'split', id, name, branches:[{id, name, count, items:[...]}]}  (분기점: 수량을 나눠 분기별로 다른 흐름)
   ===================================================================== */
window.DSIL_LIBRARY = {
  version: '2026-09-08b',
  modules: [
  ...
  flows: [
    { id: 'f-ebl-liftoff', domain: 'device', name: 'EBL 패터닝', device: '', unitLabel: '기판', unitCount: 1, description: 'PMMA 스핀코팅 → 전자빔 노광 → 현상. 리프트오프용 서브 흐름.',
      items: [
        { kind: 'module', refId: 'm-spin-pmma' },
        { kind: 'module', refId: 'm-ebl' },
        { kind: 'module', refId: 'm-develop', params: { dev: 'MIBK:IPA 1:3', time: 60 } }
      ] },
    { id: 'f-photo-liftoff', domain: 'device', name: '포토리소 패터닝 (리버설)', device: '', unitLabel: '기판', unitCount: 1, description: 'PR 스핀코팅 → 노광 → 현상. 리프트오프용 서브 흐름.',
      items: [
    ...
    { id: 'f-mos2-bg-fet-split', domain: 'device', name: 'MoS2 BG-FET 접촉 금속 · 어닐링 스플릿', device: 'MoS2 back-gate FET', unitLabel: '기판', unitCount: 4,
      description: '기판 4개를 접촉 금속(Cr/Au vs Ti/Au)으로 2:2 나누고, 합친 뒤 어닐링 온도(200 vs 300 °C)로 1:3 다시 나누는 스플릿 실험 예시.',
      items: [
        { kind: 'module', refId: 'm-substrate-clean' },
        { kind: 'module', refId: 'm-exfoliation' },
        { kind: 'module', refId: 'm-om-inspect' },
        { kind: 'split', id: 'sp-contact', name: '접촉 금속', branches: [
          { id: 'b-cr', name: 'A · Cr/Au', count: 2, items: [{ kind: 'flow', refId: 'f-metal-contact-ebl', label: '전극 형성 (Cr/Au)', params: { stack: 'Cr 5 nm / Au 50 nm' } }] },
          { id: 'b-ti', name: 'B · Ti/Au', count: 2, items: [{ kind: 'flow', refId: 'f-metal-contact-ebl', label: '전극 형성 (Ti/Au)', params: { stack: 'Ti 10 nm / Au 50 nm' } }] }
        ] },
        { kind: 'module', refId: 'm-probe-iv', label: '전기 측정 (어닐링 전)', params: { meas: 'Transfer' } },
        { kind: 'split', id: 'sp-anneal', name: '어닐링 온도', branches: [
          { id: 'b-200', name: 'X · 200 °C', count: 1, items: [{ kind: 'module', refId: 'm-anneal', params: { temp: 200 } }] },
          { id: 'b-300', name: 'Y · 300 °C', count: 3, items: [{ kind: 'module', refId: 'm-anneal', params: { temp: 300 } }] }
        ] },
        { kind: 'module', refId: 'm-probe-iv', label: '전기 측정 (어닐링 후)' }
      ] },
    ...
    { id: 'f-wirebond-pkg', domain: 'package', name: '와이어 본딩 패키징', device: 'DIP / QFN', unitLabel: '칩', unitCount: 1,
      ...
      items: [
        { kind: 'module', refId: 'm-dicing' },
        { kind: 'module', refId: 'm-die-attach' },
        { kind: 'module', refId: 'm-package' },
        { kind: 'module', refId: 'm-molding' },
        { kind: 'module', refId: 'm-pkg-inspect' }
      ] },
    { id: 'f-flipchip-pkg', domain: 'package', name: '플립칩 패키징 · 신뢰성', device: 'Flip-chip', unitLabel: '칩', unitCount: 2,
      description: '범핑 → 리플로우 접합 → 언더필 → 검사 → 신뢰성(TC vs HAST 스플릿).',
      items: [
        { kind: 'module', refId: 'm-flip-bump' },
        { kind: 'module', refId: 'm-reflow' },
        { kind: 'module', refId: 'm-underfill' },
        { kind: 'module', refId: 'm-pkg-inspect' },
        { kind: 'split', id: 'sp-rel', name: '신뢰성 시험', branches: [
          { id: 'b-tc', name: 'TC', count: 1, items: [{ kind: 'module', refId: 'm-reliability', params: { test: 'TC (열 사이클)' } }] },
          { id: 'b-hast', name: 'HAST', count: 1, items: [{ kind: 'module', refId: 'm-reliability', params: { test: 'HAST', cond: '130 °C / 85 %RH', cycles: '96 h' } }] }
        ] },
        { kind: 'module', refId: 'm-pkg-inspect', label: '시험 후 검사' }
      ] }
  ]
};
